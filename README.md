...
5. **시간 예산 가드레일**: 전체 경과 시간이 예산(기본 2150초, nbconvert 타임아웃 2400초 대비 여유
   250초)을 초과하면 이후 이미지는 즉시 `NONE` 으로 안전 처리하여, 어떤 경우에도 정량 0점으로
   이어지는 타임아웃이 발생하지 않도록 했다. 실제 4-core 환경에서 측정한 결과 예산 초과 후
   나머지 처리·CSV 저장에 걸리는 시간이 10초 내외로 매우 짧아, 원래 잡아뒀던 500초 여유를
   250초로 줄여 처리 커버리지를 더 확보했다.
